...
class Computed<T = unknown> extends Signal<T> {
  ...
  _refresh() {
    ...
    if (this._version > 0 && !this._checkIfNeedsRecompute()) {
  ...
  _checkIfNeedsRecompute() {
    if (this._lastCheckVersion !== globalVersion) {
      this._needsRecompute = this.needsToRecompute();
      this._lastCheckVersion = globalVersion;
    }
    return this._needsRecompute;
  }
...
export {
  ...
  computed,
  untracked,
  type ExternalComputed as Computed,
  type ExternalSignal as Signal
};
